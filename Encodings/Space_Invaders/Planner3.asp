...
max_x_matrix(15500).
validValue(-15500).
validValue(X):-validValue(X1), max_x_matrix(X2), X=X1+1, X<=X2.
action("MoveAction").
move("left").
move("right").
action("FireAction").
...
player(X_Next,Y,T_Next) :- player(X,Y,T), applyAction(T,"MoveAction"), actionArgument(T,"move","right"), player_move_speed(S), X_Next=X+S, X_Next<=X_Max, max_x_matrix(X_Max), T_Next=T+1, T_Next<=T_Max, maxTime(T_Max), validValue(X), validValue(S).
player(X_Next,Y,T_Next) :- player(X,Y,T), applyAction(T,"MoveAction"), actionArgument(T,"move","left"), player_move_speed(S), X_Next=X-S, X_Next>=X_Min, min_x_matrix(X_Min), T_Next=T+1, T_Next<=T_Max, maxTime(T_Max), validValue(X), validValue(S).
...
laser(X,Y_Next,S,T_Next) :- laser(X,Y,S,T), Y_Next=Y+S, T_Next=T+1, T_Next<=T_Max, maxTime(T_Max), validValue(Y), validValue(S).
laser(X,Y,200,T):-player(X,Y,T), applyAction(T,"FireAction").

% FIND ALL ENEMIES ON THE MOST LEFT/RIGHT COLUMN 
most_left_invader(X,T) :- #min{C: invaders(C,_,T)}=X, invaders(_,_,T), validValue(X).
most_right_invader(X,T) :- #max{C: invaders(C,_,T)}=X, invaders(_,_,T), validValue(X).
...
invaders_near_player(T) :- invaders(_,Y1,T), player(_,Y2,T), Y1>=Y2, Y1-Y2<=12000,validValue(Y1),validValue(Y2).
...
:-applyAction(T_Next,"MoveAction"), actionArgument(T_Next,"move","left"), player(X1,Y1,T), most_left_invader(X2,T), X1<=X2+100, T_Next=T+1,validValue(X2).
:-applyAction(T_Next,"MoveAction"), actionArgument(T_Next,"move","right"), player(X1,_,T), most_right_invader(X2,T), X1>=X2-100, T_Next=T+1,validValue(X2).
...
distance_left_column(X,T) :- invaders_direction("right"), not invaders_near_player(T), player(X1,_,T), most_left_invader(X2,T), X=X1-X2, X1>=X2,validValue(X1),validValue(X2).
distance_left_column(X,T) :- invaders_direction("right"), not invaders_near_player(T), player(X1,_,T), most_left_invader(X2,T), X=X2-X1, X1<X2,validValue(X1),validValue(X2).
...
distance_right_column(X,T) :- invaders_direction("left"), not invaders_near_player(T), player(X1,_,T), most_right_invader(X2,T), X=X1-X2, X1>=X2,validValue(X1),validValue(X2).
distance_right_column(X,T) :- invaders_direction("left"), not invaders_near_player(T), player(X1,_,T), most_right_invader(X2,T), X=X2-X1, X1<X2,validValue(X1),validValue(X2).
...
distance_player_invader(X,T) :- invaders_near_player(T), player(X1,_,T), nearest_y_invader(X2,_,T), X=X1-X2, X1>=X2,validValue(X1),validValue(X2).
distance_player_invader(X,T) :- invaders_near_player(T), player(X1,_,T), nearest_y_invader(X2,_,T), X=X2-X1, X1<X2,validValue(X1),validValue(X2).
